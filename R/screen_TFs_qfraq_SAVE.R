...
# ------------------------------------------------------------------------------
# New tidy approach v02 :)
# ------------------------------------------------------------------------------

# define prediction design
mod_form <- as.formula(loop ~ dist + strandOrientation + score_min + cor)
...
# # select example for devel
# tfDF <- filter(grp, name == "CTCF")

# k fold cross validation with 10 repeats
set.seed(3579)
...
designs <- useTF %>% 
  map(~ as.formula(paste0("loop ~ dist + strandOrientation + score_min + cor_", .x)) )
...
cv <- grp %>% 
  do(vfold_cv(data = ., V = K, repeats = 1)) 

# define desin matrix using recipes
rec <- recipe(loop ~ dist + strandOrientation + score_min + cor,
              data = grp) %>%
...
#' @param designs a list of model formulars
#' 
holdout_results_multi <- function(splits, designs) {
...
#' get coeficcients as single vector for a split of a data set and recipe
#' 
#' Source: https://github.com/topepo/rsample/blob/master/vignettes/Working_with_rsets.Rmd
#' 
#' @param splits a single split as 
...
# example <- holdout_results(cv$splits[[1]],  mod_form)

# train model on training and predict on test set
# This takes about 80sec for 3 models on 10-fold CV
cv$results <- map(cv$splits, holdout_results, mod_form)

dfCVexp <- dfCV %>% expand(name = useTF, id) %>% 
...
evalDF <- dfCVexp %>% 
  mutate(
    pred =  map(result, function(df) df[[".fitted"]]),
    label =  map(result, function(df) df[["loop"]])
  ) %>% 
  mutate(
...
mdat <- mmdata(scores = evalDF$pred,
               label = evalDF$label,
...
curveDF %>% count(modname)

autoplot(mmcurves, "PRC", show_cb = TRUE)

autoplot(evalmod(mdat))
plot(evalmod(mdat))
...
curves <- evalmod(
  scores = evalDF$pred,
  labels = evalDF$label,
  modnames = paste0("model_", evalDF$name),
  dsids = evalDF$fold,
...
# debugging evalmod and as.data.frame()
eDF <- evalDF %>% 
  filter(fold %in% 1:2) %>% 
  mutate(
    pred = map(pred, function(x) x[1:1000]),
    label = map(label, function(x) x[1:1000])
  )
...
curves <- evalmod(
  scores = eDF$pred,
  labels = eDF$label,
  modnames = paste0("model_", eDF$name),
  dsids = eDF$fold,
  posclass = levels(eDF$label[[1]])[2]
)

curveDF <- as_tibble(as.data.frame(mmcurves))
curveDF %>% count(modname)

p <- autoplot(curves, "ROC")
p
